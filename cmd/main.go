...
func main() {
	...
	createQueue := flag.Bool("createq", true, "Create SQS queue if it does not exist")
	messageType := flag.String("msgtype", "AUDIT", "Transform Audit (Server Access) Logs or Event Notifications (options AUDIT | EVENT)")
	flag.Parse()
